...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * bean的作用范围，对应Bean属性的scop
	 */
	@Nullable
	private String scope = SCOPE_DEFAULT;

	/**
	 * bean是否是抽象类,对应的属性abstract
	 */
	private boolean abstractFlag = false;

	/**
	 * 是否延迟加载,对应的属性lazy-init
	 */
	...
	/**
	 * 用来表示一个bean的实例化依靠另一个bean先实例化对一个bean属性的depend-on
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * autowire-candidate属性设置为false，这样容器在查找自动装配对象时，将不考虑该bean，既它不会被考虑当做其他bean自动装配的候选者
	 * 但是该bean本身还是可以使用自动装配来注入其他bean的，对应bean的属性autowire-candidate
	 */
	private boolean autowireCandidate = true;

	/**
	 * 自动装配时当出现多个bean候选者时，将作为首选，对应的bean属性primary
	 */
	 ...
	 * <bean id="instanceFactoryBean" class="com.yuanzf.InstanceFactoryBean" />
	...
	/**
	 * 初始化方法。对应Bean属性Init-method属性
	 */
	@Nullable
	private String initMethodName;

	/**
	 *销毁方法。对应Bean属性的story-method
	 */
	@Nullable
	private String destroyMethodName;

	/**
	 * 是否执行init-method属性，程序设置
	 */
	private boolean enforceInitMethod = true;

	/**
	 * 是否执行destory-method,该值由程序设置
	 */
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		if (count == 0) {
			...
			//标记MethodOverride暂未被覆盖，避免参数类型的检查和开销
